...
#define KEY_VNCVIEWER_HISTORI _T("Software\\teambox\\kcs\\vnc\\client\\History")

#define MENULOC_SEPARATE  0
#define MENULOC_MIXLOCAL  1
#define MENULOC_MIXREMOTE 2
...
public:
    ...
    // MetaVNC options
    bool    m_UseTransRect;     // use transparency
    bool    m_MetaWM;           // use Meta Window Manager
    bool    m_trayIcon;         // tray icon in full-screen mode
    bool    m_dragDesktop;      // enable dragging desktop
    int     m_menuLoc;          // remote menu location
	...
	static BOOL CALLBACK DlgProcGlobalOptions(HWND hwnd, UINT uMsg,
		WPARAM wParam, LPARAM lParam);
	static BOOL CALLBACK DlgProcMetaVNCOptions(HWND hwnd, UINT uMsg,
		WPARAM wParam, LPARAM lParam);
	static void Lim(HWND hwnd,int control,DWORD min, DWORD max);
	// Register() makes this viewer the app invoked for .vnc files
	static void Register();
	HWND m_hPageConnection, m_hPageGeneral, m_hPageMetaVNC, m_hTab, m_hParent, m_hWindow;
	void FixScaling();
...
private:
    ...
    void EnableMetaWM(HWND hwnd, bool enable);
