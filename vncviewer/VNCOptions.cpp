...
VNCOptions& VNCOptions::operator=(VNCOptions& s)
{
    ...
    // MetaVNC
    m_UseTransRect      = s.m_UseTransRect;
    m_MetaWM            = s.m_MetaWM;
    m_trayIcon          = s.m_trayIcon;
    m_dragDesktop       = s.m_dragDesktop;
    m_menuLoc           = s.m_menuLoc;
...
{
    TCHAR buf[256];
    LoadString(pApp->m_instance, IDS_NEEDRESTART, buf, sizeof(buf));
    MessageBox(NULL, buf, _T("Information"), MB_OK | MB_TOPMOST | MB_ICONINFORMATION);
}
...
void VNCOptions::SetFromCommandLine(LPTSTR szCmdLine) {
	...
	for (j = 0; j < i; j++) {
		if ( SwitchMatch(args[j], _T("help")) ||
			...
			ShowHelpBox(_T("MetaVNC Usage Help"));
			exit(1);
...
void VNCOptions::Save(char *fname)
{
    ...
    // MetaVNC
	saveInt("use_transrect",	    m_UseTransRect,     fname);
	saveInt("meta_wm",              m_MetaWM,           fname);
	saveInt("trayicon",             m_trayIcon,         fname);
	saveInt("dragdesktop",          m_dragDesktop,      fname);
	saveInt("menuloc",              m_menuLoc,          fname);
}
...
void VNCOptions::Load(char *fname)
{
    ...
    // MetaVNC
	m_UseTransRect =        readInt("use_transrect",    m_UseTransRect, fname) != 0;
	m_MetaWM =              readInt("meta_wm",	        m_MetaWM, fname) != 0;
	m_trayIcon =            readInt("trayicon",         m_trayIcon,     fname) != 0;
	m_dragDesktop =         readInt("dragdesktop",      m_dragDesktop,  fname) != 0;
	m_menuLoc =             readInt("menuloc",          m_menuLoc,      fname);
}
										...
										WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG: 
		{
			...
			_this->m_hPageMetaVNC = CreateDialogParam(pApp->m_instance, 
				MAKEINTRESOURCE(IDD_METAVNC_OPTIONS),
				hwndDlg,
				(DLGPROC)_this->DlgProcMetaVNCOptions,
				(LONG)_this);
			...
			SetWindowPos(_this->m_hPageGeneral, HWND_TOP, rc.left, rc.top,
						 rc.right - rc.left, rc.bottom - rc.top,
						 SWP_HIDEWINDOW);
			SetWindowPos(_this->m_hPageMetaVNC, HWND_TOP, rc.left, rc.top,
						 rc.right - rc.left, rc.bottom - rc.top,
						 SWP_HIDEWINDOW);
	...
	case WM_COMMAND:
		...
		case IDOK:
			...
			SendMessage(_this->m_hPageMetaVNC, WM_COMMAND, IDC_OK, 0);
			...
			case TCN_SELCHANGE:
				...
				case IDC_TAB:
					...
					case 1:
						...
						return 0;						
					case 2:
						ShowWindow(_this->m_hPageMetaVNC, SW_SHOW);
						SetFocus(_this->m_hPageMetaVNC);
						return 0;						
					}
					return 0;
				}
			...
			case TCN_SELCHANGING:
				...
				case IDC_TAB:
					...
					case 1:
						...
						break;
					case 2:
						ShowWindow(_this->m_hPageMetaVNC, SW_HIDE);
						break;
					}
					return 0;
				}
...
BOOL CALLBACK VNCOptions::DlgProcMetaVNCOptions(HWND hwnd, UINT uMsg,
											 WPARAM wParam, LPARAM lParam) {
	// This is a static method, so we don't know which instantiation we're 
	// dealing with. But we can get a pseudo-this from the parameter to 
	// WM_INITDIALOG, which we therafter store with the window and retrieve
	...
	case WM_INITDIALOG: 
		{
			...
			HWND hTransRect = GetDlgItem(hwnd, ID_SESSION_SET_TRANSRECT);
			SendMessage(hTransRect, BM_SETCHECK, _this->m_UseTransRect, 0);

			HWND hMetaWM = GetDlgItem(hwnd, IDC_METAWM);
			SendMessage(hMetaWM, BM_SETCHECK, _this->m_MetaWM, 0);
			_this->EnableMetaWM(hwnd, _this->m_MetaWM);

			HWND hTrayIcon = GetDlgItem(hwnd, IDC_TRAYICON);
			SendMessage(hTrayIcon, BM_SETCHECK, _this->m_trayIcon, 0);

			HWND hDragDesktop = GetDlgItem(hwnd, IDC_DRAGDESKTOP);
			SendMessage(hDragDesktop, BM_SETCHECK, _this->m_dragDesktop, 0);

			HWND hMenuLoc;
			switch (_this->m_menuLoc) {
            case MENULOC_MIXLOCAL:
				hMenuLoc = GetDlgItem(hwnd, IDC_MENULOC_MIXLOCAL);
                break;
            case MENULOC_MIXREMOTE:
				hMenuLoc = GetDlgItem(hwnd, IDC_MENULOC_MIXREMOTE);
                break;
            case MENULOC_SEPARATE:
				hMenuLoc = GetDlgItem(hwnd, IDC_MENULOC_SEPARATE);
                break;
			}
			SendMessage(hMenuLoc, BM_SETCHECK, true, 0);
	...
	case WM_COMMAND: 
		...
		case IDC_OK: 
			{		 
				HWND hTransRect = GetDlgItem(hwnd, ID_SESSION_SET_TRANSRECT);
				_this->m_UseTransRect =
					(SendMessage(hTransRect, BM_GETCHECK, 0, 0) == BST_CHECKED);

				HWND hMetaWM = GetDlgItem(hwnd, IDC_METAWM);
				_this->m_MetaWM =
					(SendMessage(hMetaWM, BM_GETCHECK, 0, 0) == BST_CHECKED);

                HWND hTrayIcon = GetDlgItem(hwnd, IDC_TRAYICON);
				_this->m_trayIcon =
					(SendMessage(hTrayIcon, BM_GETCHECK, 0, 0) == BST_CHECKED);
				
                HWND hDragDesktop = GetDlgItem(hwnd, IDC_DRAGDESKTOP);
				_this->m_dragDesktop =
					(SendMessage(hDragDesktop, BM_GETCHECK, 0, 0) == BST_CHECKED);
				
                HWND hMenuLocMixLocal = GetDlgItem(hwnd, IDC_MENULOC_MIXLOCAL);
				HWND hMenuLocMixRemote = GetDlgItem(hwnd, IDC_MENULOC_MIXREMOTE);
				if (SendMessage(hMenuLocMixLocal, BM_GETCHECK, 0, 0) == BST_CHECKED) {
					_this->m_menuLoc = MENULOC_MIXLOCAL;
				} else if (SendMessage(hMenuLocMixRemote, BM_GETCHECK, 0, 0) == BST_CHECKED) {
					_this->m_menuLoc = MENULOC_MIXREMOTE;
				} else {
					_this->m_menuLoc = MENULOC_SEPARATE;
                }
        ...
        case IDC_METAWM:
            {
                ...
                HWND hMetaWM = GetDlgItem(hwnd, IDC_METAWM);
                _this->EnableMetaWM(hwnd, SendMessage(hMetaWM, BM_GETCHECK, 0, 0) == BST_CHECKED);
            }
            return 0;
		}
		return 0;	
...
void VNCOptions::EnableMetaWM(HWND hwnd, bool enable)
{
    HWND hMenuLoc;
    hMenuLoc = GetDlgItem(hwnd, IDC_MENULOC_MIXLOCAL);
	EnableWindow(hMenuLoc, enable);
    hMenuLoc = GetDlgItem(hwnd, IDC_MENULOC_MIXREMOTE);
	EnableWindow(hMenuLoc, enable);
    hMenuLoc = GetDlgItem(hwnd, IDC_MENULOC_SEPARATE);
	EnableWindow(hMenuLoc, enable);
...
void VNCOptions::LoadOpt(char subkey[256], char keyname[256])
{
    ...
    // MetaVNC
	m_UseTransRect =        read(RegKey, "use_transrect", m_UseTransRect) != 0;
	m_MetaWM =              read(RegKey, "meta_wm", m_MetaWM) != 0;
	m_trayIcon =            read(RegKey, "trayicon",    m_trayIcon) != 0;
	m_dragDesktop =         read(RegKey, "dragdesktop", m_dragDesktop) != 0;
	m_menuLoc =             read(RegKey, "menuloc",     m_menuLoc) != 0;
...
void VNCOptions::SaveOpt(char subkey[256], char keyname[256])
{
    ...
    // MetaVNC
	save(RegKey, "use_transrect",	    m_UseTransRect      );
	save(RegKey, "meta_wm",	            m_MetaWM            );
	save(RegKey, "trayicon",            m_trayIcon          );
	save(RegKey, "dragdesktop",         m_dragDesktop       );
	save(RegKey, "menuloc",             m_menuLoc           );
