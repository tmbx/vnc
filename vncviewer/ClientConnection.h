...
#define SETTINGS_KEY_NAME "Software\\teambox\\kcs\\vnc\\client\\Settings"
...
// for MetaVNC
extern const UINT RFB_METATASKSW_CREATE;
extern const UINT RFB_METALAUNCH;
...
private:
	...
	friend class MetaTaskbar;
    MetaTaskbar *m_metaTaskbar;

	friend class MetaLauncher;
    MetaLauncher *m_metaLauncher;
    ...
    LPARAM m_prevmousepos;      // XXX workaround to ignore false mouse event

    bool m_draggingDesktop;    // in dragging the whole screen
    POINT m_dragpoint;
	...
	void ProcessKeyEvent(KeyActionSpec kas, int virtkey, DWORD keyData);
	void SendKeyEvent(CARD32 key, bool down);
    void SendKeyFocusMsg(CARD8 state);
    void SendWindowControl(CARD32 id, CARD16 control);
    void SendServerDataReq(CARD32 id, CARD16 dataType, void *data, CARD32 dataLength);
    inline void SetMenuPref() {
        /* not supported yet */
    };
    inline void SetIconPref() {
        rfbServerDataIconPrefReq iconPref;
        iconPref.bpp  = 32;
        iconPref.size = 16;
        iconPref.formatType = rfbIconFormatTypePng;
        SendServerDataReq(0, rfbServerDataIconPref, &iconPref, sizeof(iconPref));
    };
    inline void RefreshWindowState() {
        SendWindowControl(0, rfbWindowControlNone);
    };
	void SwitchOffKey();
	...
	void CheckAction(int id, bool enable);
    ...
    bool ResetTransparentRegion(int x, int y, int w, int h);
    bool SetWindowShape();

	inline bool InTransRectMode() {
		return m_enableTransRect && m_opts.m_UseTransRect;
	};

	inline bool InMetaWMMode() {
		return m_enableMetaWM && m_opts.m_MetaWM;
	};
    ...
    // ClientConnectionTransRect.cpp
	void ReadTransRect(rfbFramebufferUpdateRectHeader *pfburh);
    ...
    void ShowTaskbarButton();
    void HideTaskbarButton();
    void AddTrayIcon();
    void CheckTrayIcon();
    void RemoveTrayIcon();
    ...
    // MetaVNC
	bool    m_enableKeyFocus;
	bool    m_enableMetaWM;
	bool    m_enableTransRect;
    HRGN    m_hTransRgn; // transparent region
    RECT    m_scrnRect; // screen rect (relative to h_wnd1)
	wchar_t *m_desktopNameW; // wchar version of m_desktopName (used in MetaWM)
	...
	KeyMap m_keymap;
    // pressed key state
    std::map<UINT, KeyActionSpec> m_downKeysym;
	// mouse button state
