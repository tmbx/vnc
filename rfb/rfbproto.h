...
/*
 * Vendor signatures: standard VNC/RealVNC, TridiaVNC, TightVNC, and MetaVNC
 */

#define rfbStandardVendor "STDV"
#define rfbTridiaVncVendor "TRDV"
#define rfbTightVncVendor "TGHT"
#define rfbMetaVncVendor "META"
...
/* MetaVNC extensions, use higher values */
#define rfbChildServers 45
#define rfbWindowState 47 /* window monitor also uses */ 
#define rfbServerData  44 /* window monitor also uses */ 
/* Remember-Me extensions, use higher values */
#define rfbClientState 40
...
#define sig_rfbChildServers "CHLDSRVS"
#define sig_rfbWindowState "WND_STAT"
#define sig_rfbServerData "SRV_DATA"
#define sig_rfbClientState "CL_STATE"
...
/* MetaVNC extensions, use higher values */
#define rfbKeyFocus 49 /* obsolete */
#define rfbChildServersQuery 45
#define rfbWindowControl 46 /* server also uses for window monitor */
#define rfbServerDataReq 43 /* server also uses for window monitor */
/* Remember-Me extensions, use higher values */
#define rfbClientStateUpdate 40
#define rfbClientStateQuery  41
...
/* signatures for non-standard messages */
#define sig_rfbKeyFocus "KEYFOCUS" /* obsolete */
#define sig_rfbChildServersQuery "CHLDSRVQ"
#define sig_rfbWindowControl "WND_CTRL"
#define sig_rfbServerDataReq "SRVDATRQ"
#define sig_rfbClientStateUpdate "CLS_UPDT"
#define sig_rfbClientStateQuery  "CLS_QUER"
...
#define rfbEncodingTransRect       0xFFFFFF27 /* transparent region */
...
#define sig_rfbEncodingTransRect       "TRNSRECT"
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 * MetaVNC:
 * TransRect - Transparent rectangulars Encoding.
 * Special encoding to update transparent area.
 */

typedef struct _rfbTransRectHeader {
    CARD32 nIncRects;   /* increased transparent region */
    CARD32 nDecRects;   /* decreased transparent region */
    /* Followed by 
       rfbRectangle increct[nIncRects];
       rfbRectangle decrect[nDecRects]; */
} rfbTransRectHeader;

#define sz_rfbTransRectHeader 8
...
/*
 * Send child server addresses to client
 */
typedef struct _rfbChildServersMsg {
    CARD8 type;			/* always rfbChildServers */
    CARD8 pad1;
    CARD16 pad2;
    CARD32 nServers;
    /* followed by rfbChildServer server[nServers] */
} rfbChildServersMsg;

#define sz_rfbChildServersMsg 8

typedef struct _rfbChildServer {
    CARD32 length;
    /* followed by char hostAddress[length] */
} rfbChildServer;

#define sz_rfbChildServer 4

/*
 * Send status of windows
 * (Window Monitor -> server -> VNC client)
 */

/* NOTE:
 *   To send the application menus, there are two options;
 *   1) rfbWindowStateLauncher (rfbWindowStateMsg)
 *	 ... simple string that includes hierarchy of menu
 *   2) rfbServerDataMenuItem (rfbServerDataMsg)
 *	 ... freedesktop.org style menu that includes meta data
 */

/* status can be bit-or'ed */
#define rfbWindowStateClosed      0
#define rfbWindowStateRunning     0x1
#define rfbWindowStateMinimized   0x2
#define rfbWindowStateMaximized   0x4
#define rfbWindowStateFocused     0x8    /* window is activated (id==0 means focus out */
#define rfbWindowStateUnknown     0x4000 /* Window Monitor gone */
#define rfbWindowStateLauncher    0x8000 /* application launcher */

typedef struct {
    CARD8 type;                 /* always rfbWindowState */
    CARD8 pad1;
    CARD16 state;		/* window state */
    CARD32 id;                  /* id of window (application) */
    CARD8  nameLength;          /* length of window name */
    /* followed by char name[nameLength] */
} rfbWindowStateMsg;

#define sz_rfbWindowStateMsg 9

/*
 * Control windows
 * (VNC client -> server -> Window Monitor)
 */

#define rfbWindowControlNone 0  /* for querying status */
#define rfbWindowControlLaunch 1    /* launch application */
#define rfbWindowControlClose  2    /* close window */
#define rfbWindowControlRestore 3   /* unminimize & unmaximize window */
#define rfbWindowControlMinimize 4  /* minimize window */
#define rfbWindowControlMaximize 5  /* maximize window */
#define rfbWindowControlSetFocus 6  /* activate window (id==0 means focus out) */

typedef struct {
    CARD8 type;                 /* always rfbWindowControl */
    CARD8 pad1;
    CARD16 control;		/* control type */
    CARD32 id;                  /* id of target window (application) */
} rfbWindowControlMsg;

#define sz_rfbWindowControlMsg 8
...
typedef struct {
    CARD8 type;                 /* always rfbServerData */
    CARD8 pad1;
    CARD16 dataType;		/* data type */
    CARD32 id;                  /* id of related window or application */
    CARD32 dataLength;          /* data length */
    /* data follows */
    CARD32 clientPtr;	/* !! used only for window monitor -> server */
    /*
    union {
	// dataType : rfbServerDataMenuItem
	//   rfbServerDataMenuItem:
	//	"Desktop-File Id" + '\n' followed by
	//	content of freedesktop.org desktop entry (portion)
	char data[dataLength];
	// dataType : rfbServerDataMenuIcon, rfbServerDataWindowIcon
	rfbServerDataIcon icon;
    };
    */
} rfbServerDataMsg;

#define sz_rfbServerDataMsg 12
#define sz_rfbServerDataMsg_WM 16	/* for window monitor -> server */

/* dataType */
#define rfbServerDataMenuPref	1	/* preference of menu item */
#define rfbServerDataMenuItem	2	/* freedesktop.org desktop entry */
#define rfbServerDataIconPref	4	/* icon preference of client */
#define rfbServerDataMenuIcon	5	/* icon for menus */
#define rfbServerDataWindowIcon	6	/* icon for windows */

/* rfbServerDataMenuItem: (freedesktop.org desktop entry)
 *
 * thr first line contains the "Desktop-File Id"
 * the following keys follows;
 * - Type  (= Application)
 * - Version
 * - Encoding
 * - Name
 ...
 * - Categories
 *
 * Locale matching will be done on the server side based on the MenuPref
 */

/* data structure for rfbServerDataIcon */
typedef struct {
    CARD8 bpp;		/* bit per pixel */
    CARD8 size;		/* size of icon in pixel */
    CARD8 formatType;	/* PNG, XPM, or SVG */
    CARD8 iconData[1];
 	/* actual data length can be calculated from dataLength of
	 * rfbServerDataMsg */
} rfbServerDataIcon;

#define sz_rfbServerDataIconHdr 3

/* formatType */
#define rfbIconFormatTypePng	(1<<0)	/* PNG */
#define rfbIconFormatTypeXpm	(1<<1)	/* XPM */
#define rfbIconFormatTypeSvg	(1<<2)	/* SVG */
...
typedef struct {
    CARD8 type;                 /* always rfbServerDataReq */
    CARD8 pad1;
    CARD16 dataType;		/* data type XXX */
    CARD32 id;                  /* id of related window or application */
				/* XXX not used ?? */
    CARD32 dataLength;          /* data length */
    /* data follows */
    CARD32 clientPtr;	/* !! used only for server -> window monitor */
    /*
    union {
	// dataType : rfbServerDataMenuPref
	rfbServerDataMenuPrefReq menuPref;
	// dataType : rfbServerDataIconPref
	rfbServerDataIconPrefReq iconPref;
    };
    */
} rfbServerDataReqMsg;

#define sz_rfbServerDataReqMsg 12
#define sz_rfbServerDataReqMsg_WM 16	/* for server -> window monitor */

/* data structure for rfbServerMenuPref */
typedef struct {
    CARD8 locale[1];	/* preferred locale string */
    /* actual data length can be calculated from dataLength of
     * rfbServerDataMsg */
} rfbServerDataMenuPrefReq;

/* data structure for rfbServerIconPref */
typedef struct {
    CARD8 bpp;		/* bit per pixel */
    CARD8 size;		/* size of icon in pixel */
    CARD8 formatType;	/* client's supported format (PNG, XPM, and/or SVG) */
    CARD8 pad1;
} rfbServerDataIconPrefReq;

/*-----------------------------------------------------------------------------
 * REMEMBERME extension:
 * Send saved client state to client
 */

typedef struct _rfbClientStateMsg {
    CARD8 type;			/* always rfbClientState */
    CARD8 pad1;
    CARD16 pad2;
    CARD32 timestamp;
    CARD32 nServers;
    /* followed by rfbClientServerState server[nServers] */
} rfbClientStateMsg;

#define sz_rfbClientStateMsg 12

typedef struct _rfbClientServerState {
    CARD8  order;
    CARD8  pad1;
    CARD16 length;
    /* followed by char hostAddress[length] */
} rfbClientServerState;

#define sz_rfbClientServerState 4
...
typedef union _rfbServerToClientMsg {
    ...
    rfbChildServersMsg chs;
    rfbWindowControlMsg wnc;
    rfbWindowStateMsg wns;
    rfbServerDataReqMsg sdq;
    rfbServerDataMsg sd;
    rfbClientStateMsg cs;
...
/*-----------------------------------------------------------------------------
 * METAVNC:
 * Key Focus message (obsolete)
 * Use rfbWindowControlSetFocus instead.
 */
#define rfbKeyFocusOut  0
#define rfbKeyFocusIn   1

typedef struct {
    CARD8 type;                 /* always rfbKeyFocus */
    CARD8 state;                /* key focus state */
} rfbKeyFocusMsg;

#define sz_rfbKeyFocusMsg 2
...
typedef struct {
    CARD8 type;                 /* always rfbChildServersQuery */
} rfbChildServersQueryMsg;

#define sz_rfbChildServersQueryMsg 1
...
typedef struct _rfbClientStateQueryMsg {
    CARD8 type;			/* always rfbClientStateQuery */
    CARD8 pad1;
    CARD16 pad2;
    CARD8 username[16];
} rfbClientStateQueryMsg;

#define sz_rfbClientStateQueryMsg 20

#define rfbClientStateUpdateAll 0
#define rfbClientStateUpdateOrder 1

typedef struct _rfbClientStateUpdateMsg {
    CARD8 type;			/* always rfbClientStateUpdate */
    CARD8 pad1;
    CARD8 updateType;		/* update msg type */
    CARD8 pad2;
    CARD8 username[16];
    CARD32 timestamp;
    CARD32 nServers;
    /* if type == rfbClientStateUpdateAll,
       rfbClientServerState server[nServers] follows */
    /* if type == rfbClientStateUpdateAll,
       CARD8 order[nServers] follows */
} rfbClientStateUpdateMsg;

#define sz_rfbClientStateUpdateMsg 28
...
typedef union _rfbClientToServerMsg {
    ...
    rfbKeyFocusMsg kf;
    rfbChildServersQueryMsg chq;
    rfbWindowControlMsg wnc;
    rfbWindowStateMsg wns;
    rfbServerDataReqMsg sdq;
    rfbServerDataMsg sd;
    rfbClientStateQueryMsg csq;
    rfbClientStateUpdateMsg csu;
