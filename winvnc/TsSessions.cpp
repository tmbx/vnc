...
ProcessSessionId::ProcessSessionId(DWORD processId) {
  ...
  if (!(*_ProcessIdToSessionId)(GetCurrentProcessId(), &id))
    vnclog.Print(LL_INTERR, VNCLOG("ProcessIdToSessionId failed (error %d)\r\n"), GetLastError());
}
...
bool inConsoleSession() {
  ...
  vnclog.Print(1, "inConsoleSession() called. current console's session ID : %d, mySessionId = %d\r\n",  console.id, mySessionId.id);
...
void setConsoleSession(DWORD sessionId) {
  ...
  vnclog.Print(1, VNCLOG("sessionID passed in param : %i\r\n", sessionId));

  if (sessionId == -1)
  {  
        vnclog.Print(1, VNCLOG("sessionID == -1, setting it to %i\r\n", mySessionId.id));
        sessionId = mySessionId.id;
  }

  // Try to reconnect our session to the console
  ConsoleSessionId console;
  vnclog.Print(LL_INTINFO, VNCLOG("About to call WinStationConnect. Session ID = %d, console.ID = %d\r\n"), sessionId, console.id);
  if (!(*_WinStationConnect)(0, sessionId, console.id, L"", 0)) {
    vnclog.Print(LL_INTERR, VNCLOG("Unable to connect session to Console (error %d)\r\n"), GetLastError());
    return;
  }
