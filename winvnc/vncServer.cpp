...
vncServer::ValidPasswordsEmpty_nocache()
{
	...
	BOOL empty2 = !set2 || (strlen(plain2) == 0);
	if (empty1 && empty2)
		return TRUE;	// All passwords are empty
	...
	if (On)
	{
		...
		if (m_socketConn == NULL)
		{
			...
			if (m_autoportselect)
			{
                ...
                // Originaly, this looped from 0 to 99 to that the port
                // found would fit between the HTTP port and RFB port offset.
                // We don't use HTTP so we just don't mind if we bust the HTTP port.
				for (int i = 0; i < 200; i++)
				{
...
// MetaVNC
void
vncServer::UpdateWindowStateRefresh()
{
	vncClientList::iterator i;
	...
	for (i = m_authClients.begin(); i != m_authClients.end(); i++) {
        ...
        vncClient *pClient = GetClient(*i);
 		if (pClient->useWindowControl())
            pClient->UpdateWindowStateRefresh();
	}
}

void
vncServer::UpdateWindowState(HWND hWnd, ULONG state)
{
	vncClientList::iterator i;
	...
	for (i = m_authClients.begin(); i != m_authClients.end(); i++) {
		...
		vncClient *pClient = GetClient(*i);
 		if (pClient->useWindowControl())
            pClient->UpdateWindowState(hWnd, state);
	}
}

void
vncServer::UpdateWindowIcon(HWND hWnd)
{
	vncClientList::iterator i;
	...
	for (i = m_authClients.begin(); i != m_authClients.end(); i++) {
		...
		vncClient *pClient = GetClient(*i);
 		if (pClient->useWindowControl())
            pClient->UpdateWindowIcon(hWnd);
	}
}

void
vncServer::UpdateLauncher(ULONG id)
{
	vncClientList::iterator i;
	...
	for (i = m_authClients.begin(); i != m_authClients.end(); i++) {
		...
		vncClient *pClient = GetClient(*i);
 		if (pClient->useWindowControl())
            pClient->UpdateLauncher(id);
	}
}

void
vncServer::UpdateTpRgn(vncRegion& incRgn, vncRegion& decRgn)
{
    //vnclog.Print(LL_INTERR, VNCLOG("vncServer::updateTpRgn called\n"));
	vncClientList::iterator i;
	...
	for (i = m_authClients.begin(); i != m_authClients.end(); i++) {
		// Post the update
		vncClient *pClient = GetClient(*i);
 		if (pClient->useTransRect())
            pClient->UpdateTpRgn(incRgn, decRgn);
	}
}

void
vncServer::ClearTpRgn()
{
    vnclog.Print(LL_INTERR, VNCLOG("vncServer::ClearTpRgn called\n"));
	vncClientList::iterator i;
...
void vncServer::RefreshWindowMonitor()
{
    if (!m_desktop)
        return;

    m_desktop->RefreshWindowMonitor();
}

void vncServer::WindowControl(ULONG id, int control)
{
    if (!m_desktop)
        return;

    m_desktop->WindowControl(id, control);
}

BOOL vncServer::GetWindowName(HWND hWnd, char *name, int *len) {
    return m_desktop && m_desktop->GetWindowName(hWnd, name, len);
}

BOOL vncServer::GetLauncherMenu(ULONG id, char **m_menupath, int *m_menupathlen) {
    return m_desktop && m_desktop->GetLauncherMenu(id, m_menupath, m_menupathlen);
}

BOOL vncServer::CreateWindowIcon(HDC hMemDC, HWND hWnd, rfbServerDataIconPrefReq iconPref, rfbServerDataIcon **iconData, ULONG *iconDataLength)
{
    return m_desktop && m_desktop->CreateWindowIcon(hMemDC, hWnd, iconPref, iconData, iconDataLength);
}

BOOL vncServer::CreateLauncherIcon(HDC hMemDC, ULONG id, rfbServerDataIconPrefReq iconPref, rfbServerDataIcon **iconData, ULONG *iconDataLength)
{
    return m_desktop && m_desktop->CreateLauncherIcon(hMemDC, id, iconPref, iconData, iconDataLength);
}

BOOL vncServer::DeleteIcon(rfbServerDataIcon *iconData)
{
    return m_desktop && m_desktop->DeleteIcon(iconData);
}
