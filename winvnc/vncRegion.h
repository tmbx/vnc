...
public:
	...
	vncRegion();
	vncRegion(const vncRegion& rgn);
    vncRegion(HWND hWnd); // MetaVNC
	~vncRegion();

    vncRegion &operator=(const vncRegion &rgn);
	...
	void SubtractRect(const RECT &rect);			// Subtract a rectangle from the regions
    void Offset(int xoffset, int yoffset);  // Do OffsetRgn
	void Clear();							// Clear the current set of rectangles
	inline BOOL IsEmpty() const {			// Is the region empty?
		return region == NULL;
	}
	void Combine(const vncRegion &rgn);		// Combine with another region
	void Intersect(const vncRegion &rgn);	// Intersect with another region
	void Subtract(const vncRegion &rgn);	// Subtract another region from this one
