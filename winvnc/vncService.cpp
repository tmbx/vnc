...
BOOL
vncService::PostShareApp(DWORD pid)
{
	// Post to the WinVNC menu window
	if (!PostToWinVNC(MENU_SERVER_SHAREAPP, (WPARAM)pid, 0))
	{
		MessageBox(NULL, "No existing instance of WinVNC could be contacted", szAppName, MB_ICONEXCLAMATION | MB_OK);
		return FALSE;
	}
...
BOOL
vncService::PostPassword(enum PasswordType pwdt, char *pwd)
{
	// Post to the WinVNC menu window
	ATOM wParam = GlobalAddAtom(pwd);
	if (!PostToWinVNC(MENU_SERVER_PASSWORD, (WPARAM)wParam, pwdt))
	{
...
vncService::KillRunningCopy()
{
    ...
    // MetaVNC : kill the launcher window
	vncLauncherClose();
...
vncService::ProcessUserHelperMessage(WPARAM wParam, LPARAM lParam) {
    ...
    // MetaVNC : use this token to update special folders
    vncLauncher::updateSpecialFolders(userToken);
