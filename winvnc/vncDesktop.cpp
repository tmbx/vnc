...
const UINT RFB_WINDOW_UPDATE = RegisterWindowMessage("WinVNC.Update.Window");
...
void *vncDesktopThread::run_undetached(void *arg)
{
    ...
    // DO NOT Try to make session zero the console session.
    // See bug#173.
	//if (!inConsoleSession())
	//  setConsoleSession();
...
// TODO: suppress this message from hook when driver is active
		...
		else if (msg.message == RFB_WINDOW_UPDATE)
		{
			HWND hWnd = (HWND)msg.wParam;
            //vnclog.Print(LL_INTINFO, VNCLOG("got wndmsg 0x%X for hwnd 0x%X\n"), msg.lParam, hWnd);
			switch (msg.lParam) {
			case WM_DESTROY:
				m_desktop->m_wndmonitor->removeWindow(hWnd);
				break;
            case WM_SETFOCUS:
            case WM_KILLFOCUS:
				m_desktop->m_wndmonitor->updateTaskbar(hWnd);
                break;
            case WM_SETTEXT:
            case WM_GETTEXT:
            case WM_NCACTIVATE:
            ...
            case WM_WINDOWPOSCHANGED:
                ...
                /* not break */
            default:
				m_desktop->m_wndmonitor->updateWindow(hWnd);
				break;
            }
		}
...
//		InvalidateRect(NULL,NULL,TRUE);
	...
	// MetaVNC: create vncWindowMonitor object
	m_wndmonitor = new vncWindowMonitor(m_server, m_hwnd);
...
BOOL vncDesktop::Shutdown()
{
	...
	// MetaVNC: delete vncWindowMonitor object
	if (m_wndmonitor != NULL) {
        delete m_wndmonitor;
        m_wndmonitor = NULL;
    }
...
// Note: MetaVNC always hooks RFB_WINDOW_UPDATE;
...
vncDesktop::ActivateHooks()
{
    ...
    if (enable) {
        if (m_hooks_active && m_hooks_wndmsg_only)
            ShutdownHooks();
        m_hooks_wndmsg_only = FALSE;
    } else {
        if (m_hooks_active && !m_hooks_wndmsg_only)
            ShutdownHooks();
        m_hooks_wndmsg_only = TRUE;
    }

	if (!m_hooks_active) {
        if (!m_hooks_wndmsg_only)
		...
		if (!m_hooks_active) {
            ...
            // XXX must disable MetaVNC features
		}
	}
}
...
BOOL vncDesktop::CheckUpdates()
{
	...
	try
	{
		...
		// Has the display resolution or desktop changed? Do not check if inConsoleSession (see bug#176)
...
// window is resized
		...
		if (m_server->FullRgnRequested())
		{
            ...
            // MetaVNC: refresh transparent region
            m_wndmonitor->refreshTpRgn();
		}
...
// TODOTODO
                    ...
                    // MetaVNC: poll console windows
                    if (GetPollingFlag()) {
                        SetPollingFlag(false);
                        m_wndmonitor->pollConsoleWindows();
                    }
			}
			else
			{
...
// IMPORTANT: this order: CopyRectToBuffer, CaptureScreen, GetChangedRegion
            ...
            // MetaVNC: update transparent region
            m_wndmonitor->updateTpRgn();
		}
