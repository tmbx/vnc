...
vncRegion::vncRegion(const vncRegion& rgn)
{
	region = NULL;
	Combine(rgn);
}

vncRegion &vncRegion::operator=(const vncRegion& rgn)
{
	region = NULL;
	Combine(rgn);
	return *this;
...
vncRegion::vncRegion(HWND hWnd)
{
    ...
    int rgnval = GetWindowRgn(hWnd, region);
    if (rgnval == NULLREGION || rgnval == ERROR) {
        if (!SetRectRgn(region, rect.left, rect.top, rect.right, rect.bottom)) {
            Clear();
            return;
        }
    } else {
        if (OffsetRgn(region, rect.left, rect.top) == ERROR) {
            Clear();
            return;
        }
    }
...
void vncRegion::SubtractRect(const RECT &new_rect)
{
	HRGN newregion;
...
void
vncRegion::Combine(const vncRegion &rgn)
{
	if (rgn.region == NULL)
		return;
	if (region == NULL)
...
void
vncRegion::Intersect(const vncRegion &rgn)
{
	if (rgn.region == NULL) {
		Clear();
		return;
...
void
vncRegion::Subtract(const vncRegion &rgn)
{
	if (rgn.region == NULL)
		return;
	if (region == NULL)
...
void
vncRegion::Offset(int xoffset, int yoffset)
{
	if (region == NULL)
		return;
    if (OffsetRgn(region, xoffset, yoffset) == ERROR) {
        Clear();
        return;
    }
}
...
// get bounary box
BOOL vncRegion::GetBoundary(RECT &rect)
{
    int rgnval = GetRgnBox(region, &rect);
    return !(rgnval == NULLREGION || rgnval == 0);
}

// for debug
void vncRegion::print() {
	if (region == NULL) {
        vnclog.Print(LL_INTERR, VNCLOG("Region: null\n"));
		return;
    }
    ...
    DWORD dwSize;
    RGNDATA *pRgn;
    dwSize = GetRegionData(region, 0, NULL);
    pRgn = (RGNDATA*)GlobalAlloc(GMEM_FIXED | GMEM_ZEROINIT, dwSize);
    if (pRgn == NULL)
        return;
    if (GetRegionData(region, dwSize, pRgn) == 0) {
        vnclog.Print(LL_INTERR, VNCLOG("GetRegionData failed: size = %d\n"), dwSize);
        GlobalFree((HGLOBAL)pRgn);
        return;
    }

    vnclog.Print(LL_INTERR, VNCLOG("Region: (%d,%d)-(%d,%d), #subrect=%d\n"),
    ...
    for (DWORD i = 0; i < pRgn->rdh.nCount; i++) {
        vnclog.Print(LL_INTERR, VNCLOG("subregion %d: (%d,%d)-(%d,%d)\n"),
                     i,
                     rect->left,
                     rect->top,
                     rect->right,
