...
public:
    ...
    // MetaVNC
	BOOL useTransRect()         { return m_useTransRect; };
	BOOL useWindowControl()     { return m_useWindowControl; };
    void UpdateWindowStateRefresh();
    void UpdateWindowState(HWND hWnd, ULONG state);
    void UpdateWindowIcon(HWND hWnd);
    void UpdateLauncher(ULONG id);
    void UpdateTpRgn(const vncRegion& incRgn, const vncRegion& decRgn);
    void ClearTpRgn();
...
protected:
    ...
    // MetaVNC
	BOOL SendWindowState(ULONG id, int state, const char *windowname, int namelen);
    BOOL SendServerIconData(ULONG id, int dataType, rfbServerDataIcon *iconImage, ULONG iconDataLength);
    void SendAllWindowStates();
    BOOL SendTpIncRgn(vncRegion& rgn);
    BOOL SendTpDecRgn(vncRegion& rgn);
...
private:
	...
	// MetaVNC
	BOOL m_useTransRect;
	BOOL m_useWindowControl;
    rfbServerDataIconPrefReq m_iconPref;
    // pending transparent region
	vncRegion		m_tp_inc;
	vncRegion		m_tp_dec;
	omni_mutex		m_tp_lock;
    // pending window states
    BOOL m_wndstat_refresh;    // refresh pending
    std::map<HWND, ULONG> m_wndstat_pending;
    std::set<HWND>  m_wndicon_pending;
    std::set<ULONG> m_launcher_pending;
	omni_mutex		m_wndstat_lock;
public:
