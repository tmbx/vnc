//  Copyright (C) 2005-2007 UCHINO Satoshi. All Rights Reserved.
...
DllExport BOOL SetHook(HWND hWnd, UINT UpdateMsg, UINT CopyMsg, UINT MouseMsg, UINT WndMsg)
{
    ...
    // check only window change message ?
    m_wndonly = (UpdateMsg == NULL && CopyMsg == NULL && MouseMsg == NULL);
...
//					GetCurrentThreadId()			// DEBUG : HOOK ONLY WinVNC
	...
	if ((m_wndonly && (hCallWndHook != NULL)) ||
        ((hCallWndHook != NULL) && (hGetMsgHook != NULL) && (hDialogMsgHook != NULL)))
	{
		...
		WndChangeMessage = WndMsg;			// Save the message ID to send when window created/closed
...
// hook handler for window change message

inline BOOL HookHandleWndMsg(UINT MessageId, HWND hWnd, WPARAM wParam, LPARAM lParam)
{
	switch (MessageId) {
    // to detect changes of window text
    case WM_SETTEXT:
    case WM_GETTEXT:
    // to detect changes of visible region
	case WM_GETMINMAXINFO:
    case WM_MOVING:
    case WM_MOVE:
    case WM_SIZING:
    case WM_SIZE:
    // to detect changes of visibility
	case WM_DESTROY:
    case WM_NCACTIVATE:
	case WM_SHOWWINDOW:
    case WM_WINDOWPOSCHANGING:
    case WM_WINDOWPOSCHANGED:
		// XXX should filter out unnecessary windows
		PostMessage(hVeneto, WndChangeMessage,
                    (WPARAM)hWnd, (LPARAM)MessageId);
		break;
#if 0 // debug
    default:
        PostMessage(hVeneto, WndChangeMessage,
...
LRESULT CALLBACK CallWndProc(int nCode, WPARAM wParam, LPARAM lParam)
{
	...
	if (nCode == HC_ACTION)
	{
		...
		if (hVeneto != NULL)
		{
            ...
            HookHandleWndMsg(cwpStruct->message, cwpStruct->hwnd, cwpStruct->wParam, cwpStruct->lParam);
            if (!m_wndonly)
...
static const TCHAR szCompany[] = "teambox\\kcs\\vnc";
