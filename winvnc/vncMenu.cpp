...
const UINT MENU_SERVER_SHAREAPP = RegisterWindowMessage("WinVNC.Server.ShareApp");
const UINT MENU_SERVER_PASSWORD = RegisterWindowMessage("WinVNC.Server.ChangePwd");
...
vncMenu::vncMenu(vncServer *server)
{
	...
	m_winvnc_normal_icon = (HICON)LoadImage(hAppInstance, MAKEINTRESOURCE(IDI_WINVNC), IMAGE_ICON, 16, 16, 0);
	m_winvnc_disabled_icon = (HICON)LoadImage(hAppInstance, MAKEINTRESOURCE(IDI_DISABLED), IMAGE_ICON, 16, 16, 0);
	m_flash_icon = (HICON)LoadImage(hAppInstance, MAKEINTRESOURCE(IDI_FLASH), IMAGE_ICON, 16, 16, 0);
...
vncMenu::SendTrayMsg(DWORD msg, BOOL flash)
{
	...
	const char *title = (vncService::RunningAsService()) ?
		"MetaVNC Service - " : "MetaVNC Server - ";
	m_nid.szTip[0] = '\0';
...
/*
		...
		// The current user may have changed.
		{
			...
			if (vncService::CurrentUser((char *) &newuser, sizeof(newuser)))
			{
				...
				if (strcmp(newuser, _this->m_username) != 0)
				{
                    ...
                    // MetaVNC:
                    if (_this->m_server->MetaWindowManagerEnabled() &&
                        strlen(newuser) > 0) {
                        vncLauncher::setEnabled(TRUE);
                    } else {
                        vncLauncherClose();
                        vncLauncher::setEnabled(FALSE);
                    }

                    // refresh
                    _this->m_server->RefreshWindowMonitor();
				}
			}
		}
		return 0;
	...
	default:
		...
		if (iMsg == MENU_SERVER_SHAREALL)
		{
			...
			_this->m_server->AppShared(false);
			return 0;
		}
		...
		if (iMsg == MENU_SERVER_SHAREAPP)
		{
			DWORD pid = (DWORD)pid;
			if (pid != NULL)
			{
				_this->m_properties.HideMatchWindow();
				_this->m_server->SetAppShared(pid);
				_this->m_server->FullScreen(false);
				_this->m_server->PrimaryDisplayOnlyShared(false);
				_this->m_server->ScreenAreaShared(false);
				_this->m_server->WindowShared(false);
				_this->m_server->AppShared(true);
			}
			return 0;
		}
		if (iMsg == MENU_SERVER_PASSWORD)
		{
			char pwd[MAXPWLEN+2];
			int len = GlobalGetAtomName((ATOM)wParam, pwd, MAXPWLEN+2);
			...
			if (strcmp(pwd, "~~~~~~~~") != 0) {
                if (len == 0) {
                    ...
                    if (lParam == PWD_INTERACTIVE)
							_this->m_server->SetPassword(TRUE, crypt);
                    if (lParam == PWD_VIEWONLY)
                            _this->m_server->SetPasswordViewOnly(TRUE, crypt);
                } else {
                    vncPasswd::FromText crypt(pwd);
                    if (lParam == PWD_INTERACTIVE)
                            _this->m_server->SetPassword(TRUE, crypt);
                    if (lParam == PWD_VIEWONLY)
                            _this->m_server->SetPasswordViewOnly(TRUE, crypt);
