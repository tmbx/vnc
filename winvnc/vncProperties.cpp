...
vncProperties::Init(vncServer *server)
{
	...
	if (m_pMatchWindow == NULL) {
		m_pMatchWindow = new CMatchWindow(m_server,
                                          m_pref_ScreenAreaLeft,
                                          m_pref_ScreenAreaTop,
                                          m_pref_ScreenAreaRight,
                                          m_pref_ScreenAreaBottom);
	...
	case WM_INITDIALOG:
		{
			...
			if (_this->m_usersettings) {
				SetWindowText(hwnd, "MetaVNC Server: Current User Properties");
			} else {
				SetWindowText(hwnd, "MetaVNC Server: Default Local System Properties");
			}						
...
vncProperties::Load(BOOL usersettings)
{
	...
	m_pref_EnableMetaWindowManager = TRUE;
	m_alloweditclients = TRUE;
	m_allowshutdown = TRUE;
	m_allowproperties = TRUE;
    ...
    m_pref_FullScreenShared = TRUE;
    m_pref_PrimaryDisplayOnlyShared = FALSE;
    m_pref_ScreenAreaShared = FALSE;
    m_pref_WindowShared = FALSE;
    m_pref_SharedWindow = (HWND)0;
    {
        RECT temp;
        GetWindowRect(GetDesktopWindow(), &temp);
        m_pref_ScreenAreaLeft   = temp.left + 5;
        m_pref_ScreenAreaTop    = temp.top + 5;
        m_pref_ScreenAreaRight  = temp.right/2;
        m_pref_ScreenAreaBottom = temp.bottom/2;
    }
...
vncProperties::LoadUserPrefs(HKEY appkey)
{
	...
	m_pref_EnableMetaWindowManager=LoadInt(appkey, "EnableMetaWindowManager", m_pref_EnableMetaWindowManager);
    ...
    // Desktop sharing area
    m_pref_FullScreenShared=LoadInt(appkey, "FullScreenShared", m_pref_FullScreenShared);
    m_pref_PrimaryDisplayOnlyShared=LoadInt(appkey, "PrimaryDisplayOnlyShared", m_pref_PrimaryDisplayOnlyShared);
    ...
    m_pref_ScreenAreaLeft=LoadInt(appkey, "ScreenAreaLeft", m_pref_ScreenAreaLeft);
    m_pref_ScreenAreaTop=LoadInt(appkey, "ScreenAreaTop", m_pref_ScreenAreaTop);
    m_pref_ScreenAreaRight=LoadInt(appkey, "ScreenAreaRight", m_pref_ScreenAreaRight);
    m_pref_ScreenAreaBottom=LoadInt(appkey, "ScreenAreaBottom", m_pref_ScreenAreaBottom);
    m_pref_SharedWindow=(HWND)LoadInt(appkey, "SharedWindow", (LONG)m_pref_SharedWindow);
}
...
vncProperties::ApplyUserPrefs()
{
	...
	m_server->EnableMetaWindowManager(m_pref_EnableMetaWindowManager);
    ...
    // Desktop sharing area
    m_server->FullScreen(m_pref_FullScreenShared);
    ...
    if (m_server->ScreenAreaShared())
        m_server->SetMatchSizeFields(m_pref_ScreenAreaLeft, m_pref_ScreenAreaTop, m_pref_ScreenAreaRight, m_pref_ScreenAreaBottom);
    m_server->SetWindowShared(m_pref_SharedWindow);
}
...
vncProperties::SaveUserPrefs(HKEY appkey)
{
	...
	SaveInt(appkey, "EnableMetaWindowManager", m_server->MetaWindowManagerEnabled());
    ...
    SaveInt(appkey, "PrimaryDisplayOnlyShared", m_server->PrimaryDisplayOnlyShared());
    SaveInt(appkey, "ScreenAreaShared", m_server->ScreenAreaShared());
    SaveInt(appkey, "WindowShared", m_server->WindowShared());
    if (m_server->ScreenAreaShared()) {
        RECT screenarea = m_server->GetScreenAreaRect();
        SaveInt(appkey, "ScreenAreaLeft", screenarea.left);
        SaveInt(appkey, "ScreenAreaTop", screenarea.top);
        SaveInt(appkey, "ScreenAreaRight", screenarea.right);
        SaveInt(appkey, "ScreenAreaBottom", screenarea.bottom);
    }
    SaveInt(appkey, "SharedWindow", (LONG)m_server->GetWindowShared());
}
