...
vncClientThread::SendInteractionCaps()
{
	...
	const int MAX_SMSG_CAPS = 6;
	const int MAX_CMSG_CAPS = 9;
	const int MAX_ENC_CAPS = 15;
	...
	if (m_server->MetaWindowManagerEnabled()) {
        SetCapInfo(&smsg_list[i++], rfbWindowState, rfbMetaVncVendor);
        SetCapInfo(&smsg_list[i++], rfbServerData, rfbMetaVncVendor);
    }
	...
	SetCapInfo(&cmsg_list[i++], rfbKeyFocus,      rfbMetaVncVendor);
	if (m_server->MetaWindowManagerEnabled()) {
        SetCapInfo(&cmsg_list[i++], rfbWindowControl, rfbMetaVncVendor);
        SetCapInfo(&cmsg_list[i++], rfbServerDataReq, rfbMetaVncVendor);
    }
	...
	SetCapInfo(&enc_list[i++],  rfbEncodingTransRect,      rfbMetaVncVendor);
	...
	while (connected)
	{
		...
		case rfbSetEncodings:
			...
			msg.se.nEncodings = Swap16IfLE(msg.se.nEncodings);
			{
                ...
                // MetaVNC : disable TransRect encoding
                m_client->m_useTransRect = FALSE;
				...
				for (x = 0; x < msg.se.nEncodings; x++)
				{
                    ...
                    // MetaVNC : TransRect encoding
					if (Swap32IfLE(encoding) == rfbEncodingTransRect) {
						m_client->m_useTransRect = TRUE;
						vnclog.Print(LL_INTINFO, VNCLOG("MetaVNC TransRect encoding enabled\n"));
						continue;
					}
...
//								 m_client->m_UploadFilename, dwBytePerSecond, dwFileSize, uploadTime);
		...
		// MetaVNC
        case rfbKeyFocus: /* obsolete */
			if (!m_socket->ReadExact(((char *) &msg)+1, sz_rfbKeyFocusMsg-1)) {
				connected = FALSE;
				break;
            }
            if (msg.kf.state == rfbKeyFocusOut) {
                //vnclog.Print(LL_INTERR, VNCLOG("Focus Out\n"));
                m_server->WindowControl(0, rfbWindowControlSetFocus);
            } else {
                //vnclog.Print(LL_INTERR, VNCLOG("Focus In\n"));
            }
            break;
	    ...
	    case rfbWindowControl:
			...
			// Read the rest of the message:
			if (!m_socket->ReadExact(((char *) &msg)+1, sz_rfbWindowControlMsg-1)) {
				connected = FALSE;
				break;
            }
            m_client->m_useWindowControl = TRUE;
            vnclog.Print(LL_INTINFO, VNCLOG("windowControl: id=%Xh, control=%d\n"), Swap32IfLE(msg.wnc.id), Swap16IfLE(msg.wnc.control));
            m_server->WindowControl(Swap32IfLE(msg.wnc.id), Swap16IfLE(msg.wnc.control));
			break;
	    ...
	    case rfbServerDataReq:
        {
			...
			// Read the rest of the message:
			if (!m_socket->ReadExact(((char *) &msg)+1, sz_rfbServerDataReqMsg-1)) {
				connected = FALSE;
				break;
            }

			char *data;
			unsigned int  dataType   = Swap16IfLE(msg.sdq.dataType);
            unsigned long id         = Swap32IfLE(msg.sdq.id);
            unsigned long dataLength = Swap32IfLE(msg.sdq.dataLength);

            if (dataLength >= 0) {
                data = (char *)malloc(dataLength+1);
                if (!data || !m_socket->ReadExact(data, dataLength)) {
                    connected = FALSE;
                    if (data)
                        free(data);
                    break;
            ...
            case rfbServerDataIconPref:
                ...
                m_client->m_iconPref = *(rfbServerDataIconPrefReq *)data;
                vnclog.Print(LL_INTINFO, VNCLOG("rfbServerDataIconPref : got IconPref\r\n"));
                // XXX just refresh
                m_server->RefreshWindowMonitor();
                break;
            case rfbServerDataMenuPref:
                vnclog.Print(LL_INTINFO, VNCLOG("rfbServerDataIconPref : got MenuPref\r\n"));
                // XXX just refresh
                m_server->RefreshWindowMonitor();
                break;
            default:
                vnclog.Print(LL_INTERR, VNCLOG("rfbServerDataIconPref : ignoring unsupported data type %d\r\n"), dataType);
                break;
            }
            if (data)
                free(data);
...
vncClient::vncClient()
{
	...
	// MetaVNC
	m_useTransRect     = FALSE;
	m_useWindowControl = FALSE;
    m_iconPref.bpp     = 0;
    m_iconPref.size    = 0;
    m_iconPref.formatType = 0;
}
...
vncClient::TriggerUpdate()
{
    ...
    omni_mutex_lock l2(m_tp_lock);
    omni_mutex_lock l3(m_wndstat_lock);
	...
	if (m_updatewanted)
	{
        ...
        // send window states
        if (m_useWindowControl &&
            (m_wndstat_refresh ||
             !m_wndstat_pending.empty() ||
             !m_wndicon_pending.empty() ||
             !m_launcher_pending.empty())) {
            SendAllWindowStates();
        }
		...
		if (!m_changed_rgn.IsEmpty() ||
            ...
            (m_useTransRect && (!m_tp_inc.IsEmpty() || !m_tp_dec.IsEmpty())))
		{
...
BOOL vncClient::SendUpdate()
{
	...
	try
	{
        ...
        BOOL resetTpRgn = FALSE;    // MetaVNC: reset transparent region
        ...
        // MetaVNC: reset transparent region
        if (m_useTransRect && resetTpRgn) {
            if (!SendTpDecRgn(toBeSent))
                return FALSE;
        }

        // MetaVNC: send decreased transparent region
        if (m_useTransRect && !m_tp_dec.IsEmpty()) {
            if (!SendTpDecRgn(m_tp_dec))
                return FALSE;
            m_tp_dec.Clear();
        }
        ...
        // MetaVNC: send increased transparent region
        if (m_useTransRect && !m_tp_inc.IsEmpty()) {
            if (!SendTpIncRgn(m_tp_inc))
                return FALSE;
            m_tp_inc.Clear();
        }
...
BOOL vncClient::SendWindowState(ULONG id, int state, const char *windowname, int namelen)
{
    omni_mutex_lock l(m_sendUpdateLock);
    ...
    vnclog.Print(LL_INTINFO, VNCLOG("sending Window State: state=0x%X, id=0x%X\r\n"),  state, id);

	return (m_socket->SendQueued((char *)&wsm, sz_rfbWindowStateMsg) &&
		    (namelen > 0 ? m_socket->SendQueued((char *)windowname, namelen) : TRUE));
}

void vncClient::SendAllWindowStates()
{
    if (m_wndstat_refresh) {
        SendWindowState(0, rfbWindowStateUnknown, NULL, 0);
        m_wndstat_refresh = FALSE;
    }
    if (!m_wndstat_pending.empty()) {
        std::map<HWND, ULONG>::iterator it;
        for (it = m_wndstat_pending.begin(); it != m_wndstat_pending.end(); it++) {
            HWND hWnd   = it->first;
            ULONG state = it->second;
            char name[256];
            int len = 256;
            if (!(state & rfbWindowStateRunning) ||
                !m_server->GetWindowName(hWnd, name, &len))
                len = 0;
            SendWindowState((ULONG)hWnd, state, name, len);
        }
        m_wndstat_pending.clear();
    }
    if (!m_wndicon_pending.empty()) {
        HDC hMemDC = CreateCompatibleDC(GetDC(GetDesktopWindow()));
        std::set<HWND>::iterator it;
        for (it = m_wndicon_pending.begin(); it != m_wndicon_pending.end(); it++) {
            HWND hWnd = *it;
            rfbServerDataIcon *iconData;
            ULONG iconDataLength;
            if (m_iconPref.formatType &&
                m_server->CreateWindowIcon(hMemDC, hWnd, m_iconPref, &iconData, &iconDataLength)) {
                vnclog.Print(LL_INTINFO, VNCLOG("sending window icon for hWnd 0x%X.\r\n"), hWnd);
                SendServerIconData((ULONG)hWnd, rfbServerDataWindowIcon, iconData, iconDataLength);
                m_server->DeleteIcon(iconData);
            } else {
                vnclog.Print(LL_INTERR, VNCLOG("window icon for hWnd 0x%X is not sent.\r\n"), hWnd);
            }
        }
        m_wndicon_pending.clear();
        DeleteObject(hMemDC);
    }
    if (!m_launcher_pending.empty()) {  // lancher and launcher icon
        HDC hMemDC = CreateCompatibleDC(GetDC(GetDesktopWindow()));
        std::set<ULONG>::iterator it;
        for (it = m_launcher_pending.begin(); it != m_launcher_pending.end(); it++) {
            ULONG id = *it;
            char *menuPath;
            int menuPathLen;
            if (m_server->GetLauncherMenu(id, &menuPath, &menuPathLen)) {
                SendWindowState((ULONG)id, rfbWindowStateLauncher,
                                menuPath, menuPathLen);

                rfbServerDataIcon *iconData;
                ULONG iconDataLength;
                if (m_iconPref.formatType &&
                    m_server->CreateLauncherIcon(hMemDC, id, m_iconPref, &iconData, &iconDataLength)) {
                    vnclog.Print(LL_INTINFO, VNCLOG("sending launcher icon for id %u.\r\n"), id);
                    SendServerIconData((ULONG)id, rfbServerDataMenuIcon, iconData, iconDataLength);
                    m_server->DeleteIcon(iconData);
				} else {
                    vnclog.Print(LL_INTERR, VNCLOG("launcher icon for id %u is not sent.\r\n"), id);
				}
            }
        }
        m_launcher_pending.clear();
        DeleteObject(hMemDC);
    }
}

BOOL vncClient::SendServerIconData(ULONG id, int dataType, rfbServerDataIcon *iconData, ULONG dataLength)
{
    omni_mutex_lock l(m_sendUpdateLock);

    if (dataType != rfbServerDataMenuIcon	&&
        dataType != rfbServerDataWindowIcon)
        return FALSE;
    ...
    sdm.dataLength = Swap32IfLE(dataLength);

	return (m_socket->SendQueued((char *)&sdm, sz_rfbServerDataMsg) &&
            m_socket->SendQueued((char *)iconData, dataLength));
}

BOOL vncClient::SendTpIncRgn(vncRegion& incRgn)
{
    omni_mutex_lock l(m_sendUpdateLock);
    RECT tpBoundary;
	rectlist tpIncList;
    if (incRgn.IsEmpty() || !incRgn.Rectangles(tpIncList)) {
        vnclog.Print(LL_INTERR, VNCLOG("SendTransRect : Failed to get transparent region\n"));
        return FALSE;
    }
    incRgn.GetBoundary(tpBoundary);

	// Create the message header
	rfbFramebufferUpdateRectHeader hdr;
	hdr.r.x = Swap16IfLE(tpBoundary.left);
	hdr.r.y = Swap16IfLE(tpBoundary.top);
	hdr.r.w = Swap16IfLE(tpBoundary.right  - tpBoundary.left);
    hdr.r.h = Swap16IfLE(tpBoundary.bottom - tpBoundary.top);
	hdr.encoding = Swap32IfLE(rfbEncodingTransRect);
	// Now send the message;
	if (!m_socket->SendQueued((char *)&hdr, sizeof(hdr)))
        return FALSE;

    // send TransRect message header
    rfbTransRectHeader trh;
    trh.nIncRects = Swap32IfLE(tpIncList.size());
    trh.nDecRects = 0;
	if (!m_socket->SendQueued((char *)&trh, sizeof(trh)))
        return FALSE;

    rectlist::iterator i;

    // inc region
    for (i = tpIncList.begin(); i != tpIncList.end(); i++) {
        rfbRectangle rfbRect;
        rfbRect.x = Swap16IfLE(i->left - tpBoundary.left);
        rfbRect.y = Swap16IfLE(i->top  - tpBoundary.top );
        rfbRect.w = Swap16IfLE(i->right  - i->left);
        rfbRect.h = Swap16IfLE(i->bottom - i->top);
        if (!m_socket->SendQueued((char *)&rfbRect, sizeof(rfbRect)))
            return FALSE;
    }
...
BOOL vncClient::SendTpDecRgn(vncRegion& decRgn)
{
    omni_mutex_lock l1(m_sendUpdateLock);
    omni_mutex_lock l2(m_tp_lock);
    RECT tpBoundary;
	rectlist tpDecList;
    if (decRgn.IsEmpty() || !decRgn.Rectangles(tpDecList)) {
        vnclog.Print(LL_INTERR, VNCLOG("SendTransRect : Failed to get transparent region\n"));
        return FALSE;
    }
    decRgn.GetBoundary(tpBoundary);
    ...
    rfbTransRectHeader trh;
    trh.nIncRects = 0;
    trh.nDecRects = Swap32IfLE(tpDecList.size());
	if (!m_socket->SendQueued((char *)&trh, sizeof(trh)))
        return FALSE;
    ...
    // dec region
    for (i = tpDecList.begin(); i != tpDecList.end(); i++) {
        rfbRectangle rfbRect;
        rfbRect.x = Swap16IfLE(i->left - tpBoundary.left);
        rfbRect.y = Swap16IfLE(i->top  - tpBoundary.top );
        rfbRect.w = Swap16IfLE(i->right  - i->left);
...
void
vncClient::UpdateWindowStateRefresh()
{
    omni_mutex_lock l(m_wndstat_lock);
    m_wndstat_refresh = TRUE;
    m_wndstat_pending.clear();
    m_wndicon_pending.clear();
    m_launcher_pending.clear();
}
...
void
vncClient::UpdateWindowState(HWND hWnd, ULONG state)
{
    omni_mutex_lock l(m_wndstat_lock);
    m_wndstat_pending.insert(std::map<HWND, ULONG>::value_type(hWnd, state));
}

void
vncClient::UpdateWindowIcon(HWND hWnd)
{
    omni_mutex_lock l(m_wndstat_lock);
    m_wndicon_pending.insert(hWnd);
}

void
vncClient::UpdateLauncher(ULONG id)
{
    omni_mutex_lock l(m_wndstat_lock);
    m_launcher_pending.insert(id);
}

void
vncClient::UpdateTpRgn(const vncRegion& incRgn, const vncRegion& decRgn)
{
    if (incRgn.IsEmpty())
        vnclog.Print(LL_INTINFO, VNCLOG("UpdateTpRgn : incRgn is empty!\n"));
    if (decRgn.IsEmpty())
        vnclog.Print(LL_INTINFO, VNCLOG("UpdateTpRgn : decRgn is empty!\n"));

    omni_mutex_lock l(m_tp_lock);
	m_tp_inc.Subtract(decRgn);
	m_tp_dec.Subtract(incRgn);
	m_tp_inc.Combine(incRgn);
	m_tp_dec.Combine(decRgn);
    if (m_tp_inc.IsEmpty())
        vnclog.Print(LL_INTINFO, VNCLOG("UpdateTpRgn : m_tp_inc is empty!\n"));
    if (m_tp_dec.IsEmpty())
        vnclog.Print(LL_INTINFO, VNCLOG("UpdateTpRgn : m_tp_dec is empty!\n"));
}

void
vncClient::ClearTpRgn()
{
    omni_mutex_lock l(m_tp_lock);
	m_tp_inc.Clear();
	m_tp_dec.Clear();
